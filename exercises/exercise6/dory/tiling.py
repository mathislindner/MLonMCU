...
class Tiling():
    ...
    def get_tiling_conv1d(self, X, Y, W,
        ...
        ds_y = self.BitOut
        ds_W = self.BitW
        fs1 = self.filter_size
        p_left = self.padding
        p_right = self.padding
        n_in = self.x_shape[0]
        n_out = self.out_ch
        name_include = []
        # L3 tiling
        ...
        w_out = 1
        if dilation > 1:
            h_out = int(np.floor((h_in - ((fs1 - 1)*dilation) + p_left + p_right + (self.stride - 1)) / self.stride))
        else:
            h_out = int(np.floor((h_in - (fs1 - 1) + p_left + p_right + (self.stride - 1)) / self.stride))
        ...
        if tiling is not None:          
            ...
            x_tot_str = '[%dx%d]' % (n_in, h_in)
            y_tot_str = '[%dx%d]' % (n_out, h_out)
            W_tot_str = '[%dx%dx%d]' % (n_out, n_in, fs1)

            x_tot_size_str = "%.2f KiB" % (1. / 1024. / 8. * (ds_x * n_in * h_in )) if ds_x * \
                n_in * h_in > 1024 else '%d B' % (ds_x * n_in * h_in * 1 / 8.)
            y_tot_size_str = '%.2f KiB' % (1. / 1024. / 8. * (ds_y * n_out * h_out )) if ds_y * \
                 n_out * h_out * w_out > 1024 else '%d B' % (ds_y * n_out * h_out  * 1 / 8.)
            ...
            out_dim1 = n_out_temp * h_out_temp

            n_in_temp = self.x_shape[0]
            h_in_temp = self.x_shape[-2]
            in_dim1 = n_in_temp * h_in_temp
