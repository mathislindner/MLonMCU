...
def print_template_layer_1D(x, y_gold, W,
    ...
    if h_out * stride + fs1 - 1 - stride + 1 > h_in:
        if (h_out * stride + fs1 - 1 - stride + 1 - h_in) == padding_left:
            padding_r = 0
    ...
    tk['x_tile_size_byte'] = int(math.ceil(ds_x * tile_n_in * tile_h_in / 8.0))
    ...
    tk['y_tile_size_byte'] = int(math.ceil(tile_n_out * tile_h_out * ds_y / 8.0))
    ...
    if has_bias == 1:
        tk['l2_off_bias'] = 0
    if n_in == tile_n_in and h_in == tile_h_in:
        x_buffer_size = int(math.ceil(ds_x * tile_n_in * tile_h_in / 8.0))
    else:
        x_buffer_size = 2 * int(math.ceil(ds_x * tile_n_in * tile_h_in / 8.0))
    if n_in == tile_n_in and h_in == tile_h_in and n_out == tile_n_out:
        y_buffer_size = int(math.ceil(ds_y * tile_n_out * tile_h_out / 8.0))
        W_buffer_size = int(math.ceil(ds_W * tile_n_out * tile_n_in * fs1 / 8.0))
    else:
        y_buffer_size = 2 * int(math.ceil(ds_y * tile_n_out * tile_h_out / 8.0))
    ...
    if tk['tile_dim_h'] == 1:
        ...
        tk['x_tile_size_h_last'] = h_in - tile_h_in + tk['conv_overlap1'] + padding_left
    elif tk['tile_dim_h'] == 3:
        tk['x_tile_size_h_last'] = h_in - tile_h_in - (tile_h_in - tk['conv_overlap1'] - padding_left) + tk['conv_overlap1']
    else:
        tk['x_tile_size_h_last'] = h_in - tile_h_in - (tile_h_in - tk['conv_overlap1'] - padding_left) - (tk['tile_dim_h'] - 3) * (tile_h_in - tk['conv_overlap1']) + tk['conv_overlap1']
    ...
    if 'L2' in test_location:
        ...
        with open(save_string, "w") as f:
            f.write(s)
    return l2_dim_input, l2_dim_output, l2_dim_weights, l2_dim_k, l2_dim_lambda, tk['nof'], buffer_l1_all, n_out, h_out
